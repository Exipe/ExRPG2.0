...
export type WeatherEffect = keyof typeof weatherDefinitions;
export const WEATHER_EFFECTS: ReadonlyArray<WeatherEffect> = Object.keys(weatherDefinitions) as WeatherEffect[];
...
export const initWeather = async (gl: WebGL2RenderingContext, resPath: string) => {
    ...
    return new WeatherEffectHandler(particleTexture);
}

export class WeatherEffectHandler {
    ...
    public setEffect(engine: Engine, effect: WeatherEffect) {
        this.layers = weatherDefinitions[effect].map(def =>
